...
export default function Slider() {
  ...
  return (
    <div className="w-full h-[650px] w-full m-auto py-16 relative tablet:mt-20 desktop-one:mt-0 desktop-two:mt-44">
      <div
        style={{ backgroundImage: `url(${slides[currentIndex].url})` }}
        className="w-full h-full  bg-center bg-cover "
      ></div>
      <div onClick={prevSlide} className="absolute top-[50%] -translate-x-0 translate-y-[-50%] left-10 text-2xl flex justify-center items-center h-11 w-11 rounded-full p-2 bg-white text-white cursor-pointer">
        <img src={ArrowL}  alt="" />
      </div>
      <div  onClick={nextSlide} className="absolute top-[50%] -translate-x-0 translate-y-[-50%] right-10 text-2xl flex justify-center items-center h-11 w-11 rounded-full p-2 bg-white text-white cursor-pointer">
        <img src={ArrowR} alt="" />
      </div>
      <div className=" absolute top-[50%] -translate-x-0 translate-y-[-50%] w-96 left-40 text-white ">
        <h3 className="mb-3">{slides[currentIndex].title}</h3>
        <h2 className="text-6xl mb-3">{slides[currentIndex].bigTitle}</h2>
        <p className="mb-3">{slides[currentIndex].text}</p>
